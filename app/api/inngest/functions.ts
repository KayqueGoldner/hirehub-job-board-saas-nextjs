...
export const sendPeriodicJobListings = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    while (currentDay < totalDays) {
      ...
      if (recentJobs.length > 0) {
        await step.run("send-email", async () => {
          ...
          const userTempEmail = process.env.RESEND_USER_TEMP_EMAIL!;

          await resend.emails.send({
            from: "HireHub <onboarding@resend.dev>",
            to: [userTempEmail], // TODO: change to event data email
    ...
    return {
      userId,
      message: "Completed 30 day job listing notifications",
    };
  },
);
